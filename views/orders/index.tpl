      ...
      <div class="row">
        <div class="col-sm-12">
          <section class="panel mail-box">
            <header class="panel-heading"> 订单管理 / 总数：{{.countOrder}}<span class="tools pull-right"><a href="javascript:;" class="fa fa-chevron-down"></a>
              ...
              <section class="mail-list" style="margin-top:6px;">
                ...
                <ul class="list-group ">
                  ...
                  <li class="list-group-item"> <span class="pull-left chk">
                    ...
                    </span> <a class="thumb pull-left" href="/user/show/{{$v.Userid}}"> <img src="{{getAvatarUserid $v.Userid}}" style="width:22px;"> </a> <a href="{{$v.Url}}"> <small class="pull-right text-muted">{{getDateMH $v.Created}}</small> <strong>{{getRealname $v.Userid}}</strong> <span>{{getOrderType $v.Type}}了&nbsp;&nbsp;{{$v.Title}}</span> </a> </li>
                  {{else}}
                  <li class="list-group-item text-center">目前还没有任何订单</li>
                  {{end}}
                </ul>
...
$(function(){
	...
	$('#deleteMsg').on('click', function(){
		...
		$.post('/order/ajax/delete', {ids:str},function(data){
			dialogInfo(data.order)
			if (data.code) {
